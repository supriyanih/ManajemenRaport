...
class Guru extends MX_Controller{
    ...
    public function index()
	{
		...
		if(!empty($cek)){
			...
			$tot_hal = $this->mdl_guru->manualQuery($text);		
			...
			$d['data'] = $this->mdl_guru->manualQuery($text);
			
			
			$d['content'] = $this->load->view('guru/view_guru', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('guru/form_guru', $d, true);		
			$this->load->view('home/home',$d);
		}else{
                    redirect('login');
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$data = $this->mdl_guru->manualQuery($text);
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
                                        ...
                                        $d['nama']	   =$db->nama;
                                        $d['tempat']	   =$db->tempat;
                                        $d['tgl_lahir'] =$this->mdl_guru->tgl_sql($db->tgl_lahir);
                                        $d['jenkel']	   =$db->jenkel;
                                        $d['telpon']   =$db->telpon;
                                        $d['email']     =$db->email;
                                        $d['alamat']    =$db->alamat;
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->mdl_guru->manualQuery("DELETE FROM staff WHERE nip='$id'");
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
                                        ...
                                        $up['tgl_lahir']   =$this->mdl_guru->tgl_sql($this->input->post('tgl_lahir'));
				...
				$data = $this->mdl_guru->getSelectedData("staff",$id);
				if($data->num_rows()>0){
					$this->mdl_guru->updateData("staff",$up,$id);
					echo 'Update data Sukses';
				}else{
					$this->mdl_guru->insertData("staff",$up);
    ...
    public function InfoGuru()
	{
		...
		if(!empty($cek)){
			...
			if($tabel->num_rows()>0){
				...
				$d['nip']	   ='';
                        ...
                        $d['content'] = $this->load->view('guru/profile', $d, true);		
			$this->load->view('home/home',$d);
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('guru/edit_profil', $d, true);		
			$this->load->view('home/home',$d);
