...
class Mdl_login extends CI_Model{
	...
	public function ambilBln($tgl){
		$exp = explode('-',$tgl);
		$tgl = $exp[1];
		$bln = $this->mdl_login->getBulan($tgl);
	...
	public function tgl_indo($tgl){
			...
			$bulan = $this->mdl_login->getBulan(substr($tgl,5,2));
        ...
        public function getLoginData($usr,$psw)
	{
		...
		$q_cek_login = $this->db->get_where('staff', array('username' => $u, 'password' => $p));
		if(count($q_cek_login->result())>0)
		{
			foreach($q_cek_login->result() as $qck)
			{
					...
					{
						$sess_data['logged_in'] = 'sudahlogin';
